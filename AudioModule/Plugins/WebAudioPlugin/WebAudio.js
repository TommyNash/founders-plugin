var WebAudio = {
    ...
    InitializeAudioSystem: function(volume) {
        ...
        try {
            ...
            logMessage("Web Audio API not supported:" + e, "error", "Web Audio");
            return;
        }
            ...
            try {
                if (document.hidden) {
                    if (typeof WebAudio.PauseAll === 'function') {
                        ...
                        logMessage("WebAudio.PauseAll is not available", "warning", "Web Audio");
                    }
                } else {
                    if (typeof WebAudio.UnpauseAll === 'function') {
                        WebAudio.UnpauseAll.call(WebAudio);
                    } else {
                        logMessage("WebAudio.UnpauseAll is not available", "warning", "Web Audio");
                    }
                }
            } catch (error) {
                logMessage("Error in visibilitychange handler:" + error, "error", "Web Audio");
            }
        });
        ...
        window.addEventListener('blur', () => {
            try {
                ...
                logMessage("Error in blur handler:" + error, "error", "Web Audio");
            }
        });
        ...
        window.addEventListener('focus', () => {
            try {
                ...
                logMessage("Error in focus handler:" + error, "error", "Web Audio");
            }
        });
    },
    ...
    LoadSound: function(key, path, defaultVolume, pitch, spatial = false) {
        return new Promise((resolve, reject) => {
            ...
            fetch(path)
                ...
                .catch(error => {
                    logMessage(`Error loading sound ${key}:` + error, "error", "Web Audio");
    ...
    PlaySound: function(key, startTime = 0, volume = 1, pitch = 1, priority = 0, position = null) {
        ...
        if (!WebAudio.manageActiveSounds(key, priority)) {
            logMessage(`Cannot play sound ${key}: too many active sounds`, "warning", "Web Audio");
            return -1;
        }
        ...
        try {
            ...
            logMessage(`Error playing sound ${key}:` + error, "error", "Web Audio");
            WebAudio.activeSounds.delete(key);
            return -1;
        }
    },
    ...
    SetListenerPosition: function(x, y, z) {
        ...
        if (![x, y, z].every(Number.isFinite)) {
            console.warn("SetListenerPosition: non-finite values ​​obtained", {x, y, z});
            return;
        }
    ...
    SetListenerOrientation: function(x, y, z, forwardX, forwardY, forwardZ, upX, upY, upZ) {
        ...
        if (!values.every(Number.isFinite)) {
            console.warn("SetListenerOrientation: non-finite values ​​obtained", {
                position: {x, y, z},
    ...
    SetPosition: function(key, x, y, z, id) {
        // Проверка на конечные числа
        if (![x, y, z].every(Number.isFinite)) {
            console.warn("SetPosition: non-finite values ​​obtained", {key, x, y, z, id});
            return;
        }
    ...
    PauseAll: function() {
        try {
            if (!this.sounds) {
                logMessage("WebAudio.sounds is not available" + error, "error", "Web Audio");
                return;
            }
            ...
            logMessage("Error in PauseAll:" + error, "error", "Web Audio");
        }
    },
    ...
    UnpauseAll: function() {
        try {
            ...
            logMessage("Error in UnpauseAll:" + error, "error", "Web Audio");
        }
    }
}; 