...
namespace FoundersPlugin.Modules
{
    ...
    public class AudioModule : MonoBehaviour
    {
        ...
        private void LoadSettings()
        {
            try
            {
                var settingsPath = "AudioModuleSettings";
                var settings = Resources.Load<TextAsset>(settingsPath);
                        
                        if (settings != null)
                        {
                    var json = JObject.Parse(settings.text);
                    defaultVolume = json["defaultVolume"]?.Value<float>() ?? 1.0f;
                    cacheLifetimeSeconds = json["cacheLifetimeSeconds"]?.Value<int>() ?? 300;
                    maxCacheItems = json["maxCacheItems"]?.Value<int>() ?? 100;
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"Ошибка загрузки настроек AudioModule: {ex.Message}");
            }
        }
            ...
            try
        {
            ...
            WebGLAudioBridge.InitializeAudioSystem(defaultVolume);
                    isInitialized = true;
                    Debug.Log("WebGL аудио система инициализирована успешно");
                }
            #endif
            }
            catch (Exception ex)
            {
                Debug.LogError($"Ошибка инициализации WebGL аудио системы: {ex.Message}");
                isInitialized = false;
            }
        }
