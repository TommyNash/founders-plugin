...
namespace FoundersKit.Modules
{
    ...
    public class AdsModule : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            {
                isInitialized = true;
                Log.Success("Module initialized", moduleAttribute);
            }
            catch (Exception ex)
            {
                Log.Error($"Error initializing AudioModule: {ex.Message}", moduleAttribute);
            }
        }
        #endregion
        ...
        public void ShowFullscreen()
        {
            if (CanShowAd())
            {
                Log.Info($"ShowFullscreen at {Time.time:F1}", moduleAttribute, scope: LogScope.Local);
                ResetAdTimer();
#if UNITY_EDITOR
            ...
            else
            {
                ...
                Log.Warning($"Cannot show fullscreen ad. Cooldown: {timeLeft:F1}s remaining", moduleAttribute, scope: LogScope.Local);
            }
        }
        ...
        public void ShowRewarded(int rewardId)
        {
            ...
            Log.Info($"ShowRewarded(ID={rewardId}) at {Time.time:F1}", moduleAttribute, scope: LogScope.Local);
#if UNITY_EDITOR
